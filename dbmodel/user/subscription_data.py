...
class SubscriptionGroup(SubscriptionGroupModel):
    ...
    def add_item(self, user):
        ...
        s.commit()
        # s.close()
        error, resp = SubscriptionMember().add_item(self, user, user)
        if error:
            return True, resp
...
class SubscriptionMember(SubscriptionMemberModel):
    ...
    @staticmethod
    def get_item_titular(user):
        item = s.query(SubscriptionMember).filter(
            and_(SubscriptionMember.id_usuario == user, SubscriptionMember.titular == True)).first()
        if not item:
    ...
    def add_item(self, group, user, updated_by):
